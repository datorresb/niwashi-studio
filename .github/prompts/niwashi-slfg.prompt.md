---
name: niwashi-slfg
description: "Run the full autonomous narrative pipeline: DISCOVER → RESEARCH → WIREFRAME → parallel BUILD → auto BUILD↔REVIEW loop → smoke test → HARVEST. Minimal human intervention after wireframe approval."
agent: niwashi
argument-hint: "<concept to explain> [--cycles N] [--audience <audience>]"
---

Read and follow the instructions in [skills/niwashi-slfg/SKILL.md](../../skills/niwashi-slfg/SKILL.md).

User wants to run the autonomous SLFG pipeline. $input
